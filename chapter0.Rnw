...
hmean=mean(HeightInWholeInches$Height)
@
...
<<summation, echo=F, include=T, results='asis'>>=
xsum=sprintf(" %d +",orgdata$Height)
xsum[Nobs]=sprintf("%d $\r\n$= %d $",orgdata$Height[Nobs],hsum)
...
HeightInWholeInches$Deviation=round(HeightInWholeInches$Height-hmean,1)
...
mcenterchart=ggplot(orgdata,aes(x=Observation,y=Height))+geom_point() + theme_tufte() + 
...
The deviations are both positive and negative, and the sum of the deviations is zero, so this statistic alone is not suitable for further analysis. If we square the deviations, the sum is no longer zero; in fact, the sum of the squared deviations is the \textbf{Variance}\marginnote{The \textbf{round(Variance,1)} is a measure of variability or spread $Var(X)$, often denoted by $\sigma^2$}. The variance of our dataset is \Sexpr{variancex} square inches. To get back to our original unit of inches, we take the square root of the variance, \Sexpr{round(stddevx,1)}, or \textbf{Standard Deviation}\marginnote{The \textbf{Standard Deviation} of X, $StdDev(X)$, often denoted $\sigma$, is the standard measure of spread used in statistical analysis.}
...
dtable=xtable(orgdata[,c(6,1,4,5)],caption = "Deviations")
rws <- seq(1, (nrow(orgdata)-1), by = 2)
col <- rep("\\rowcolor[gray]{0.95}", length(rws))
print(dtable,include.rownames=F, scalebox=0.95,booktabs = TRUE, 
   add.to.row = list(pos = as.list(rws), command = col),sanitize.text.function=function(x){x})
