...
opts_chunk$set(fig.path='figure/graphics-', cache.path='cache/graphics-', fig.align='center', dev='pdf', fig.width=5, fig.height=5, fig.show='hold', cache=FALSE, par=TRUE)
...
orgdata$Observation=(1:nrow(orgdata))
...
cat(HeightInWholeInches$Height)
...
mediann1=floor((n+1)/2)
mediann2=ceiling((n+1)/2)
...
Q3n1=floor((n+1)*3/4)
Q3n2=ceiling((n+1)*3/4)
Q3=(HeightInWholeInches$Height[Q1n2]+
          HeightInWholeInches$Height[Q1n1])/2
@
...
The \textbf{mean}\marginnote{The \textbf{Mean} of a data set refers to the arithmetic mean of the values} height is \Sexpr{mean} inches. The mean is the center that we will use to further examine the ``spread'' of the values.

<<center-chart,echo=FALSE,include=TRUE,fig.cap="Heights in Observation order with Mode (Green), Median (Blue) and Mean (Red) lines",fig.height=2.5>>=
...
The range and quartiles are used to create a \textbf{boxplot}.
\end{document}