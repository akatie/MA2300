...
dtable=xtable(orgdata[,cols],caption = "Deviations")
rws <- seq(1, (nrow(orgdata)-1), by = 2)
col <- rep("\\rowcolor[gray]{0.95}", length(rws))
align(dtable) <- "|ccr|c|c|"
tt=capture.output(print(dtable,include.rownames=F, scalebox=0.95,booktabs = TRUE, add.to.row = list(pos = as.list(rws), command = col),sanitize.text.function=function(x){x}))
